...
public final class DisplayContent {
    ...
    public DisplayContent(String message) {
        initBase(message);
        x = (GameActivity.WORLD_WIDTH-layout.width)/2;
        y = ((GameActivity.WORLD_HEIGHT+layout.height) /2) + 38;
    }

    public DisplayContent(String message, float px, float py) {
        initBase(message);
        x = px;
        y = py;
    }
